...
/**
 * Le hub Studio IA n'affiche plus d'emoji d'interface · rollout icônes (retour
 * proprio #2). Le 🔒 de l'état verrouillé passe à <Icon lock>.
 */
...
describe('Hub Studio · plus aucun emoji d’interface', () => {
  it('le fichier ne porte aucun pictogramme', () => {
    const trouves = [...new Set(SRC.match(PICTO) ?? [])];
    expect(trouves, `pictogramme(s) encore dans studio/page.tsx : ${trouves.join(' ')}`).toEqual([]);
  });
