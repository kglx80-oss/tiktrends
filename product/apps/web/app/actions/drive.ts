...
import { googleConfigured, googleAccessToken, driveListFolders, storageFromEnv, syncDriveAssets, type DriveFolder } from '@tiktrends/integrations';
...
export interface DriveState { available: boolean; connected: boolean; folderId: string | null; folderName: string | null; syncedAt: string | null }

export async function getDriveState(): Promise<DriveState> {
  const s = await getSession();
  const available = googleConfigured();
  if (!s || !db) return { available, connected: false, folderId: null, folderName: null, syncedAt: null };
  ...
  return { available, connected: !!w?.tok, folderId: w?.fid ?? null, folderName: w?.fname ?? null, syncedAt: w?.at ? w.at.toISOString() : null };
}

export async function listDriveFoldersAction(): Promise<{ folders?: DriveFolder[]; error?: string }> {
  const g = await guard();
  ...
  try {
    ...
    return { folders: await driveListFolders(token) };
