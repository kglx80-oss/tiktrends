...
/**
 * Trouve ou crée le chemin persona → désir → angle.
 *
 * Tout arrive `proposed` : une créa poussée depuis le Studio ne décide pas de la
 * taxonomie de la marque, elle propose un rattachement que l'humain corrige. Sans
 * cette précaution, la carte se remplirait d'angles fantômes en quelques jours.
 */
async function ensureGraphPath(opts: {
...
async function nextVariant(conceptId: string): Promise<string> {
  if (!db) return 'v1';
  const [row] = await db.select({ n: sql<number>`count(*)` }).from(schema.ads).where(eq(schema.ads.conceptId, conceptId));
  return `v${Number(row?.n ?? 0) + 1}`;
}
