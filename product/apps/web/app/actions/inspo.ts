...
export async function saveAd(input: { platform: string; externalId: string; snapshot: InspoAd }): Promise<void> {
  ...
  await db.insert(schema.savedAds)
    .values({ workspaceId: s.workspaceId, userId: s.user.id, platform: input.platform, externalId: input.externalId, snapshot: input.snapshot })
    .onConflictDoNothing();
}
...
export async function followBrand(input: { platform: string; name: string; externalId?: string; logoUrl?: string }): Promise<void> {
  ...
  await db.insert(schema.followedBrands)
    .values({ workspaceId: s.workspaceId, platform: input.platform, name: input.name, externalId: input.externalId || null, logoUrl: input.logoUrl || null })
    .onConflictDoNothing();
}
